...
class _HomeScreenState extends State<HomeScreen> {
  final TextEditingController _docCodeController =
      TextEditingController(); //handles the i/p of doc code to open
  List<String> recentDocs =
      []; //stores recent document codes locally using SharedPreferences
  ...
  Future<void> _loadRecentDocs() async {
    ...
    setState(() {
      //triggers a rebuild of the widget with updated data (updating ui state)
  ...
  Future<void> _saveRecentDoc(String docId) async {
    final prefs =
        await SharedPreferences.getInstance(); //loads the preferences from device storage

    if (!recentDocs.contains(docId)) {
      //if doc id is not inthe recentDocs list
      recentDocs.insert(
        0,
        docId,
      ); //insert at beginning, thus it keeps the most recently accessed documents at the top
      ...
      await prefs.setStringList(
        'recentDocs',
        recentDocs,
      ); //saves the updated recentDocs list to local storage using the key 'recentDocs'
    }
    ...
    await showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        actions: [
          ...
          TextButton(
            onPressed: () async {
              //asynchronous function so we can use await to perform firestore and sharedpreferences operations
              ...
              final docRef = FirebaseFirestore
                  .instance //points to a new Firestore document inside the "documents" collection
              ...
              await docRef.set({
                'title': title,
                'content': [
                  //initializes with an empty line in Quill editor format
                  {'insert': '\n'},
                ],
                ...
                'createdBy': FirebaseAuth
                    .instance
                    .currentUser!
                    .uid, //stores current user's UID
                'isPublic': false,
                'sharedWith': [],
              });

              await _saveRecentDoc(
                docId,
              ); //adds the docId to recent documents in SharedPreferences
              _openDocumentById(
                docId,
              ); //navigate to editor screen for newly created doc
            },
  ...
  void _openDocumentById(String docId) {
    //go to editor screen with given doc id
    Navigator.push(
      context,
  ...
  void _openDocument() {
    //called when user enters a code and clicks "Open document"
